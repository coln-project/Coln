...
As an example, the simplest case one might imagine is adding another element `d:V`
to instance `G` and an edge `e3:E` with `e3 src: c` and `e3 tgt: d`. And if we
want to query the `reachable` relation, we just need to run the axioms on our
existing data against the newly added data, and repeat until we reach the fixpoint.
The new `reachable(c, d), reachable(a, d), reachable(b, d)` should be added
to the instance data as an operation `assert` as well (?).
...
\begin{tabular}{cccccccccccccc}\toprule
  ...
  - & - & d & - & - & - & - & add & - & V & - & - & - & - \\
  - & - & e & - & - & - & - & add & - & E & - & - & - & - \\
  - & - & reachable & - & - & - & - & assert & - & RelArgs & (a, b) & - & - & - \\\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}
  ...
  e.g.\ \verb|d| would be the name of the element added\@. (int,actor) is not relevant here
  \item opId is the unique operation identifier. Omitted here for now
  ...
  of the relation in this single column. I do not see big problems with it (?)
  \item succ is for dependency tracking, omitted here.
\end{itemize}
