...
instance DefEq (TyV K) where
  defEq a a' = case (a, a') of
    ...
    (VPi pv dom cod, VPi pv' dom' cod') -> do
      unless (pv == pv') $
        throwUnequalTys a a' $
         Just $ "different pi variants:" <+> pretty pv <+> "and" <+> pretty pv'
      defEq dom dom'
  ...
  defEq n n' = do
    unless (n.head == n'.head) $
      throwUnequalEls (VNeu n) (VNeu n') $
        Just $ "different heads for neutral:" <+>
          prtHead n.head <+> "and" <+> prtHead n'.head
...
instance DefEq Spine where
  defEq sp sp' = case (sp, sp') of
    ...
    (SProj sq x, SProj sq' x') -> do
      defEq sq sq'
      unless (x == x') $ throwUnequalSpines sq sq' $
        Just $ "fields are not equal:" <+> pretty x <+> "and" <+> pretty x'
