...
foreign import javascript unsafe "new TextDecoder('utf-8', {fatal: true})"
  js_newTextDecoder :: IO JSVal

foreign import javascript unsafe "new TextEncoder()"
...
foreign import javascript unsafe "$1.decode(new Uint8Array(__exports.memory.buffer, $2 + $3, $4))"
  js_decodeUtf8 ::
    JSVal ->
    ByteArray# ->
    Int -> -- offset
...
foreign import javascript unsafe "$1.decode(new Uint8Array(__exports.memory.buffer, $2, $3))"
  js_decodeUtf8Ptr ::
    JSVal ->
    Ptr CChar ->
    Int ->
...
foreign import javascript unsafe "$1.encodeInto($2, new Uint8Array(__exports.memory.buffer, $3, $4)).written"
  js_encodeUtf8 ::
    JSVal ->
    JSString ->
