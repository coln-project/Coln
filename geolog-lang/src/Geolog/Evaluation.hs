...
teleEq v v' (x:xs) (TVCons a fs) (TVCons a' fs') =
    let vx = proj v x
        vx' = proj v' x
        fs'' = teleEq v v' xs (fs vx) (fs' vx') in
        TVCons (VTmEq (proj v x) a (proj v' x) a') (\_ -> fs'')
...
behavesAs (VTmEq v a v' a') = case (behavesAs a, behavesAs a') of
  (BehavesAs b, BehavesAs b') -> case (b, b') of
     (VPi pv dom (Clo x f), VPi pv' dom' (Clo x' f')) ->
        if pv == pv'
        then BehavesAs $ VPi pv dom (
          Clo x $ \vx -> VPi pv dom' (
              Clo x' $ \vx' -> VPi pv (VTmEq vx dom vx' dom') (
                  Clo "pf" $ \_ ->
...
coerceCons (x:xs) (TVCons a f) (TVCons a' f') v =
  let vx = proj v x
      vx' = coerce (proj v x) a a' in
    FCons x vx' (coerceCons xs (f vx) (f' vx') v)
...
coerce v a a' = case (behavesAs a, behavesAs a') of
  (BehavesAs b, BehavesAs b') -> case (b, b') of
    (VU u, VU u') | u == u' -> v
    (VPi pv dom (Clo _ f), VPi pv' dom' (Clo _ f')) | pv == pv' ->
      VLam $ Clo "x" (\vx' ->
                    let vx = coerce vx' dom' dom in
                    coerce (app v vx) (f vx) (f' vx'))
