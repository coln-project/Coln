...
pub fn sccs(adjacency: &[Vec<usize>]) -> Vec<Vec<usize>> {
...
mod tests {
    ...
    /// Checks that `sccs` partitions the nodes and that no component is
    /// emitted before one it has an edge into.
    fn assert_well_formed(adjacency: &[Vec<usize>], sccs: &[Vec<usize>]) {
        let mut component_of = vec![None; adjacency.len()];
        sccs.iter().enumerate().for_each(|(idx, members)| {
