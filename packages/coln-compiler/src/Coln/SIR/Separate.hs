...
instance Separate (V.El Theory) (S.El Theory) where
  separate n = \case
    ...
    V.Code SSetU a -> S.Multi (separate n a)
    V.Code SPropU a -> S.Holds (asPred n a)
    V.Lam SSetTheory clo -> S.Lam (separateClo n clo)
    V.Cons fields -> S.Cons $ separate n <$> fields
...
predAt :: CtxLen -> V.Ty Set -> V.El Set -> S.Pred
predAt n = \case
  V.EltOf x args -> \v ->
  ...
  V.Record rt -> \v -> do
    let go [] _ = []
        go ((x, k):rest) vs = do
          let v' = V.proj v x
          (x, predAt n (k vs) v') : (go rest (vs :> Pair SSet v'))
    S.And $ fromList $ go (toList rt.fieldTypes) rt.capture
  V.BuiltinTy _ -> \_ -> S.truePred
  V.Eq at lhs rhs -> \_ ->
    S.Eq (shapeOf at) (separate n lhs) (separate n rhs)

asPred :: CtxLen -> V.Ty Set -> S.Pred
asPred n = \case
  V.EltOf x args -> S.Atom x Nothing (separate n <$> args)
  V.Record rt -> do
    let v = V.local (FId n)
    let go [] _ = []
        go ((x, k):rest) vs = do
          let v' = V.proj v x
          (x, asPred n (k vs)) : (go rest (vs :> Pair SSet v'))
    S.And $ fromList $ go (toList rt.fieldTypes) rt.capture
  V.BuiltinTy _ -> S.truePred
  V.Eq at lhs rhs -> S.Eq (shapeOf at) (separate n lhs) (separate n rhs)
    
instance Separate (V.Ty Set) S.Query where
  separate n a =
    S.Query (shapeOf a) (S.Abs Nothing (predAt (n + 1) a (V.local (FId n))))
