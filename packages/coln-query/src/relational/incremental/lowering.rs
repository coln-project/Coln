...
//! from.
//!
//! Expressed as a single [`TransformationRule`] over the shared
//! [rewriter](crate::optimizer::rewrite), so what lives here is the translation
//! itself and nothing about how the plan is walked. The rule is bottom-up,
//! which is what keeps [`fold`] simple: by the time a join is offered,
//! its operands (and any multi-way join nested in them) are already lowered,
//! so the fold can move them into the chain as they are.
//!
...
use crate::{
    ...
    optimizer::rewrite::{RewriteDriver, Rewritten, TransformationRule},
    relational::expr::{
...
pub fn fold_multi_way_joins(plan: QueryIr) -> Result<QueryIr, LoweringError> {
    ...
    // post-condition — "no multi-way join survives" — and one the pass verifies
    // is worth more than one its rule scheduling merely implies. The
    // alternative is discovering the gap as an `unimplemented!()` part-way
    // through building a circuit.
    let survived = walk::pre_order(&lowered)
        .filter_map(Node::as_rel)
        .any(|rel_expr| rel_expr.kind() == RelKind::MultiWayEquiJoin);
    if survived {
        return Err(LoweringError::new(
            "A multi way equi join survived lowering, so the DBSP backend has no way to build \
             a circuit for it. Either the fold declined it or a later rule put one back",
        ));
    }
...
/// Replaces one [`MultiWayEquiJoinExpr`] with a chain of binary joins.
struct MultiWayJoinFold;

impl TransformationRule for MultiWayJoinFold {
    fn name(&self) -> &'static str {
        "multi-way-join-fold"
    }

    /// The one node kind this translation is about. Everything that decides
    /// *how* a join folds — the order, whether its keys can be named — needs
    /// the join's contents, so it lives in [`fold`] rather than here.
    ...
    fn apply(&mut self, node: RelExpr) -> Result<Rewritten, RewriteError> {
        match node {
            RelExpr::MultiWayEquiJoin(join) => fold(*join).map(Rewritten::Changed),
            other => Ok(Rewritten::Unchanged(other)),
        }
    }
}
...
/// Folds one join, whose operands the [rewriter](crate::optimizer::rewrite)
/// has already lowered on the way down.
fn fold(join: MultiWayEquiJoinExpr) -> Result<Expr, RewriteError> {
...
mod tests {
    ...
    use crate::{
        ...
        relational::expr::{OutputExpr, OutputKind, SinkId},
    };
    ...
    fn assert_equi_join(expr: &Expr) -> &EquiJoinExpr {
        match expr {
            Expr::Relational(RelExpr::EquiJoin(join)) => join,
            other => {
                panic!("Expected a relational expression containing an equi join, got {other:?}")
            }
        }
    }
    ...
    fn compares_a_later_occurrence_against_the_accumulated_copy() {
        ...
        assert_eq!(outer.right, relation("r2"));
        assert_eq!(
            key_names(assert_equi_join(&outer.left)),
    ...
    fn keeps_the_projection_on_the_outermost_join() {
        ...
        assert_eq!(assert_equi_join(&outer.left).attributes, None);
    }
    ...
    fn folds_an_empty_join_condition_into_a_chain_of_products() {
        ...
        assert!(assert_equi_join(&outer.left).on.is_empty());
    }

    #[test]
    fn lowers_a_join_nested_inside_another_one() {
        // The rule is bottom-up, so the inner join is already a chain by the
        // time the outer one is offered — one round, no scheduling involved.
        let inner =
        ...
        match &lowered {
            Expr::Relational(rel) => match rel {
                RelExpr::Output(output) => {
                    assert_eq!(
                        key_names(assert_equi_join(&output.relation)),
                        vec![("x".to_string(), "x".to_string())]
                    );
                }
